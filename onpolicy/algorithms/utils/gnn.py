...
class TransformerConvNet(nn.Module):
    ...
    def processAdj(self, adj: Tensor):
        ...
        index = adj.nonzero(as_tuple=True) # tuple of 2 definiing node ids of non-zero edges
        edge_attr = adj[index] # all 40 edges being non-zero and their edge attribute
...
class RelationalConvNet(nn.Module):
    def __init__(
        self,
        input_dim: int,
        num_embeddings: int,
        ...
        Args:
            ...
            edge_dim (int, optional):
                ...
                considered in `EmbedConv`. Defaults to 1.
        """
        super(RelationalConvNet, self).__init__()
        self.active_func = [nn.Tanh(), nn.ReLU()][use_ReLU]
        self.num_heads = num_heads
        self.concat_heads = concat_heads
        self.edge_dim = edge_dim
        ...
        #     edge_dim=edge_dim,
        # )
        self.embed_layer = nn.Linear(input_dim, hidden_size)
    ...
    def forward(self, node_obs: Tensor, adj: Tensor, agent_id: Tensor):
        ...
        # forward pass through embedConv
        # x = self.embed_layer(x, edge_index, edge_attr) # change
        ...
        # forward pass conv layers
        # x = self.active_func(self.gnn2(x, edge_index, edge_attr))
    ...
    def processAdj(self, adj: Tensor):
        ...
        """
        assert adj.dim() >= 2 and adj.dim() <= 3
        assert adj.size(0) == adj.size(1) # TODO this should through an error and should be adj.size(-2) == adj.size(-3)
        # filter far away nodes and connection to itself - CHANGED SO IT DOES NOT FILTER FAR AWAY NODES
        # connect_mask = ((adj < self.max_edge_dist) * (adj > 0)).float()
        # adj = adj * connect_mask
        ...
        if len(index) == 4:
            raise Exception('index has one dimmension too much, i.e. batch is included')
            batch = index[0] * adj.size(-1)
            index = (batch + index[1], batch + index[2])

        return torch.stack(index[:2], dim=0), edge_attr
